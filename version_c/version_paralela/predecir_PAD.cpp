...
int main() {
    ...
    string pythonScriptPath = "scritps_python/generar_matrices_bloque.py";  
    ...
    for(int i=0; i<NUMERO_DE_PROCESOS; i++) {
        ...
        int ultimoMedidor = min(firstFile + numFilesPerProcess, static_cast<int>(files.size()));
    ...
    string pythonScriptPath = "scritps_python/generar_imagen_de_archivo.py";  
    vector<string> files;
    for(auto& p: filesystem::directory_iterator("datos")) {
    ...
    for(int i=0; i<NUMERO_DE_PROCESOS; i++) {
        ...
        int lastFile = min(firstFile + numFilesPerProcess, static_cast<int>(files.size()));
        for(int j=firstFile; j<lastFile; j++) {
            string command = "python " + pythonScriptPath + " " + files[j];
            system(command.c_str());
        }
    }
    */
